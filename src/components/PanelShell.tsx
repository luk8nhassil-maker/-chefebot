      ...
      <style>{`
        ...
        @media (min-width: 768px) {
          .ps-sidebar {
            ...
            background: var(--background); border-right: 1px solid var(--surface);
            z-index: 100; overflow-y: auto; padding: 24px 0;
          }
          .ps-sidebar-brand { padding: 0 20px 20px; border-bottom: 1px solid var(--surface); margin-bottom: 8px; }
          .ps-sidebar-btn {
            display: flex; align-items: center; gap: 10px;
            padding: 10px 20px;
            font-family: 'Archivo', sans-serif; font-size: 13px; font-weight: 800;
            color: var(--foreground-muted); background: transparent; border: none;
      ...
      <nav className="ps-sidebar">
        <div className="ps-sidebar-brand">
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
            {logoError ? (
              <div style={{ width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Pizza size={20} color="var(--foreground)" aria-hidden="true" /></div>
            ) : (
              <Image
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 900, letterSpacing: "-0.4px", color: "var(--foreground)" }}>ChefeBot</div>
              <div style={{ fontSize: 10, color: "var(--foreground-muted)", fontWeight: 700 }}>Painel operacional</div>
            </div>
          </div>
        </div>
        ...
        <button className={`ps-sidebar-btn${onPedidos ? " ps-active" : ""}`} onClick={() => router.push("/pedidos")}>
          ...
          Pedidos
          {pedidosCount > 0 && <span style={{ marginLeft: "auto", minWidth: 20, height: 20, borderRadius: 10, background: "var(--primary)", color: 'var(--primary-foreground)', fontSize: 10, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 5px" }}>{pedidosCount}</span>}
        </button>
        ...
        {showGestaoNav && (
          <>
            <div style={{ padding: "12px 12px 4px" }}>
              ...
              <div style={{ padding: "0 8px", fontSize: 10, fontWeight: 800, letterSpacing: "0.4px", color: "var(--border-strong)", textTransform: "uppercase" }}>Gestão</div>
            </div>
