...
describe("ClientBottomNav — bolha do item ativo não é cortada (overflow)", () => {
  test("o item ativo recebe a classe active, e o CSS embutido garante overflow:visible tanto no .cbn-nav (fixed+transform) quanto no .cbn-nav-inner (pill arredondada) — sem isso o Chromium corta o topo do círculo elevado (translateY(-16px)) mesmo sem overflow:hidden explícito em nenhum dos dois", () => {
  ...
  test("a bolha ativa cresce (36px→54px) e sobe (translateY(-16px)) — geometria que só fica visível com overflow:visible nos ancestrais", () => {
    const html = renderToStaticMarkup(<ClientBottomNav active="inicio" />);
    const activeRule = html.match(/\.cbn-item\.active \.cbn-icon-circle\{[^}]*\}/)?.[0] ?? "";
    expect(activeRule).toContain("width:54px");
    expect(activeRule).toContain("height:54px");
    expect(activeRule).toContain("transform:translateY(-16px)");
  });
});
