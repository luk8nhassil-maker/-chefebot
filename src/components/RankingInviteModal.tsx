...
export default function RankingInviteModal({
  ...
  description = "Entre no ranking e acompanhe sua posição. Seu nome e telefone aparecem de forma protegida.",
    ...
    <div className="cf-post-order-invite-backdrop" role="presentation">
      <section className="cf-post-order-invite" role="dialog" aria-modal="true" aria-labelledby="post-order-invite-title">
        <div className="cf-post-order-invite-visual" aria-hidden="true">
          ...
          <h2 id="post-order-invite-title">E se o presente for seu?</h2>
        </div>
        <p className="invite-eyebrow"><Gift size={14} aria-hidden="true" /> {eyebrow}</p>
        <p className="invite-description">{description}</p>
        <div className="invite-privacy"><span aria-hidden="true">✓</span><strong>Privacidade protegida</strong><small>usamos apenas o necessário.</small></div>
        <button type="button" className="invite-primary" onClick={onParticipar}><span className="invite-free">GRÁTIS</span>Participar<ChevronRight size={18} aria-hidden="true" /></button>
