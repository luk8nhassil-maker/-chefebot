      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm">
            <Link href="/" className={linkClass("dashboard")}>Dashboard</Link>
            <Link href="/simulador" className={linkClass("simulador")}>Simulador</Link>
            {user && (user.role === "admin" || user.role === "atendente") && (
              <Link href="/pedidos" className={linkClass("pedidos")}>Pedidos</Link>
            )}
            {user?.role === "admin" && (
              <Link href="/relatorios" className={linkClass("relatorios")}>Relatórios</Link>
            )}
            {user ? (
