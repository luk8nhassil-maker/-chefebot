...
function buildReceipt(session: BotSession): string {
  const lines = session.cart.map((item, i) => {
    const border =
      item.border !== "Sem borda" ? ` + ${item.border}` : "";
...
export function processMessage(
  ...
  switch (session.step) {
    case "welcome": {
      ...
      const saltyList = MENU.saltyFlavors
        .map((f, i) => `  ${i + 1}. ${f}`)
        .join("\n");
      const sweetList = MENU.sweetFlavors
        .map((f, i) => `  ${MENU.saltyFlavors.length + i + 1}. ${f}`)
        .join("\n");

      return {
        messages: [
          `Olá! 👋 Bem-vindo à *Chefe da Pizza*! 🍕\n\nSou o ChefBot e vou te ajudar a fazer seu pedido.\n\nQual o tamanho da pizza?\n\n  1. Pequena (P) — R$ 35,00\n  2. Média (M) — R$ 40,00\n  3. Grande (G) — R$ 50,00\n  4. Família (F) — R$ 55,00\n\nDigite o número ou a letra (P, M, G ou F):`,
        ],
    ...
    case "size": {
      ...
      if (!size) {
        return {
          messages: [
            `Não entendi. Por favor escolha o tamanho:\n\n  1. Pequena (P) — R$ 35,00\n  2. Média (M) — R$ 40,00\n  3. Grande (G) — R$ 50,00\n  4. Família (F) — R$ 55,00`,
          ],
          session,
        };
      }
    ...
    case "flavor": {
      ...
      if (!flavor) {
        ...
        return {
          messages: [
            `Sabor não encontrado. Escolha pelo número ou nome:\n\n🧀 *Salgadas*\n${saltyList}\n\n🍫 *Doces*\n${sweetList}`,
          ],
          session,
        };
      }
    ...
    case "border": {
      ...
      const border = wantsBorder
        ? "Borda recheada"
        : "Sem borda";
      const borderPrice = wantsBorder
        ? getBorderPrice(session.currentSize!)
        : 0;
    ...
    case "add_more": {
      ...
      if (addMore) {
        return {
          messages: [
            `Qual o tamanho da próxima pizza?\n\n  1. Pequena (P) — R$ 35,00\n  2. Média (M) — R$ 40,00\n  3. Grande (G) — R$ 50,00\n  4. Família (F) — R$ 55,00`,
          ],
          session: { ...session, step: "size" },
        };
      }
    ...
    case "delivery_type": {
      ...
      if (isDelivery) {
        const list = MENU.neighborhoods
          .map((n, i) => `  ${i + 1}. ${n.group} — ${formatCurrency(n.fee)}`)
          .join("\n");
        return {
          messages: [
      ...
      const payList = MENU.payments
        .map((p, i) => `  ${i + 1}. ${p}`)
        .join("\n");
      return {
        messages: [
          `Ótimo! Você retirará na loja. 🏠\n\nQual a forma de pagamento?\n\n${payList}`,
        ],
        session: {
          ...session,
          step: "payment",
    ...
    case "neighborhood": {
      ...
      if (!isNaN(num) && num >= 1 && num <= MENU.neighborhoods.length) {
        ...
        found = MENU.neighborhoods.find(
          (n) =>
            n.areas.some((a) => a.toLowerCase() === lower) ||
      ...
      if (!found) {
        ...
        return {
          messages: [
            `Bairro não encontrado. Escolha pelo número ou nome:\n\n${list}`,
          ],
          session,
        };
      }
      ...
      return {
        messages: [
          `Entrega para região *${found.group}*. Taxa: ${formatCurrency(found.fee)} 🛵\n\nQual a forma de pagamento?\n\n${payList}`,
        ],
        session: {
          ...session,
          step: "payment",
          neighborhood: found.areas[0],
    ...
    case "payment": {
      ...
      if (!payment) {
        ...
        return {
          messages: [
            `Forma de pagamento não reconhecida. Escolha:\n\n${payList}`,
          ],
          session,
        };
      }
    ...
    case "done": {
      return {
        messages: [
          `Olá novamente! Quer fazer um novo pedido? 🍕\n\nQual o tamanho da pizza?\n\n  1. Pequena (P) — R$ 35,00\n  2. Média (M) — R$ 40,00\n  3. Grande (G) — R$ 50,00\n  4. Família (F) — R$ 55,00`,
        ],
        session: {
          step: "size",
          cart: [],
