...
const inputStyle: React.CSSProperties = {
  ...
  padding: '13px 14px', color: 'var(--foreground)', fontSize: 16, fontWeight: 700,
    ...
    fetch('/api/admin/salao/config')
      ...
      .then((data) => {
        if (data) {
          ...
          setWhatsapp(formatarTelefoneExibicao(data.whatsappAtendimento))
        }
      })
  ...
  async function salvarWhatsapp() {
    ...
    try {
      const r = await fetch('/api/admin/salao/config', {
        ...
        body: JSON.stringify({ whatsappAtendimento: whatsapp }),
      })
      if (r.status === 401) { router.push('/login?callbackUrl=/admin/salao'); return }
      const data = await r.json().catch(() => ({}))
      if (!r.ok || !data?.ok) {
        setErroWhatsapp(data?.error || 'Não foi possível salvar agora. Tente de novo.')
        return
      }
      setConfigurado(true)
      ...
      <div style={{ padding: 24, maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 20, fontFamily: "'Archivo', sans-serif" }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--foreground-secondary)', margin: '4px 0 0' }}>
            Cadastre o WhatsApp do atendimento do salão para receber contato direto por mensagem.
          </p>
        </div>
        ...
        {carregando ? (
          ...
          <>
            ...
            <div style={cardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: 'var(--foreground)' }}>
                  {configurado ? 'WhatsApp configurado' : 'WhatsApp ainda não configurado'}
                </p>
              </div>
            ...
            <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                <label style={labelStyle} htmlFor="salao-whatsapp">Número com DDD</label>
                <input
                  id="salao-whatsapp"
                  className="cbInput"
                  value={whatsapp}
                  onChange={(e) => { setWhatsapp(e.target.value); setMensagemWhatsapp('') }}
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <button
                  ...
                  disabled={salvando || !whatsapp.trim()}
                  style={{ ...botaoPrimario, opacity: salvando || !whatsapp.trim() ? 0.5 : 1 }}
                >
