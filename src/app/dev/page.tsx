...
export default function DevPage() {
  ...
  const excluirPadrao = async (original: string) => {
    ...
    try {
      ...
      setMensagem('✅ Padrão excluído!'); setTimeout(() => setMensagem(''), 3000)
    } catch {
      setMensagem('❌ Erro ao excluir.'); setTimeout(() => setMensagem(''), 3000)
    }
  }
  ...
  const salvarEdicao = async (original: string) => {
    try {
      ...
      setEditando(null)
      setNovoValor('')
      setMensagem('✅ Padrão atualizado!'); setTimeout(() => setMensagem(''), 3000)
    } catch {
      setMensagem('❌ Erro ao salvar.'); setTimeout(() => setMensagem(''), 3000)
    }
  }
  ...
  const limparLogs = async () => {
    ...
    try {
      ...
      setLogs([])
      setMensagem('✅ Logs limpos!'); setTimeout(() => setMensagem(''), 3000)
    } catch {
      setMensagem('❌ Erro.'); setTimeout(() => setMensagem(''), 3000)
    }
  ...
  const corNivel = (nivel: string) => {
    if (nivel === 'erro') return { bg: 'rgba(252,129,129,0.1)', border: 'rgba(252,129,129,0.3)', text: '#fc8181', icon: '❌' }
    if (nivel === 'aviso') return { bg: 'rgba(246,173,85,0.1)', border: 'rgba(246,173,85,0.3)', text: '#f6ad55', icon: '⚠️' }
    return { bg: 'rgba(99,179,237,0.1)', border: 'rgba(99,179,237,0.3)', text: '#63b3ed', icon: 'ℹ️' }
  }
      ...
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 28 }}>
          <div style={{ background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.25)', borderRadius: 14, padding: 18 }}>
            <p style={{ color: 'rgba(167,139,250,0.6)', fontSize: 11, margin: '0 0 6px', fontWeight: 700, textTransform: 'uppercase' }}>🧠 Padrões</p>
          ...
          <div style={{ background: 'rgba(99,179,237,0.08)', border: '1px solid rgba(99,179,237,0.25)', borderRadius: 14, padding: 18 }}>
            <p style={{ color: 'rgba(99,179,237,0.6)', fontSize: 11, margin: '0 0 6px', fontWeight: 700, textTransform: 'uppercase' }}>🔄 Usos</p>
          ...
          <div style={{ background: logs.some(l => l.nivel === 'erro') ? 'rgba(252,129,129,0.08)' : 'rgba(104,211,145,0.08)', border: `1px solid ${logs.some(l => l.nivel === 'erro') ? 'rgba(252,129,129,0.25)' : 'rgba(104,211,145,0.25)'}`, borderRadius: 14, padding: 18 }}>
            <p style={{ color: logs.some(l => l.nivel === 'erro') ? 'rgba(252,129,129,0.6)' : 'rgba(104,211,145,0.6)', fontSize: 11, margin: '0 0 6px', fontWeight: 700, textTransform: 'uppercase' }}>🔴 Erros</p>
        ...
        {aba === 'padroes' && (
          <div>
            <div style={{ marginBottom: 20 }}>
              <input type="text" placeholder="Buscar padrão..." value={busca} onChange={e => setBusca(e.target.value)}
            ...
            <h2 style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, fontWeight: 700, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
              Biblioteca — {padroesFiltrados.length} resultado{padroesFiltrados.length !== 1 ? 's' : ''}
            </h2>
            {padroesFiltrados.length === 0 ? (
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 32, textAlign: 'center' }}>
                <p style={{ color: 'rgba(255,255,255,0.2)', margin: 0, fontSize: 15 }}>
                  {padroes.length === 0 ? 'Nenhum padrão aprendido ainda.' : 'Nenhum padrão encontrado.'}
                </p>
              </div>
            ) : (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', gap: 12 }}>
                          <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: 11, margin: 0 }}>🔄 {p.vezes}x</p>
                          <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: 11, margin: 0 }}>📅 {new Date(p.criadoEm).toLocaleDateString('pt-BR')}</p>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 6, marginLeft: 8 }}>
                        <button onClick={() => { setEditando(p.original); setNovoValor(p.interpretado) }} style={{ background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.3)', color: '#a78bfa', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', fontSize: 12 }}>✏️</button>
                        <button onClick={() => excluirPadrao(p.original)} style={{ background: 'rgba(252,129,129,0.1)', border: '1px solid rgba(252,129,129,0.3)', color: '#fc8181', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', fontSize: 12 }}>🗑️</button>
                      </div>
                    </div>
                    {editando === p.original && (
                      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                        ...
                        <button onClick={() => salvarEdicao(p.original)} style={{ background: 'rgba(104,211,145,0.15)', border: '1px solid rgba(104,211,145,0.4)', color: '#68d391', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>✅</button>
                        <button onClick={() => setEditando(null)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontSize: 13 }}>✕</button>
                      </div>
                    )}
                  </div>
                ))}
        ...
        {aba === 'logs' && (
          <div>
            ...
            {logs.length === 0 ? (
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 32, textAlign: 'center' }}>
                <p style={{ color: 'rgba(255,255,255,0.2)', margin: 0, fontSize: 15 }}>Nenhum log registrado. Sistema saudável! ✅</p>
              </div>
            ) : (
