...
export default function RastrearPage({ params }: PageProps) {
  ...
  // e sem o contato dinâmico do WhatsApp.
  ...
  // usado por /pedido/pagamento/[token] (GET /api/pedido-app/pagamento-pix),
  // que devolve whatsappPizzaria sanitizado de config:pizzaria independente
  // do estado do pagamento — nunca um número fixo no código, nunca uma
  // segunda lógica de normalização. Só busca quando há statusToken (sem
  // token, o botão simplesmente não aparece). Número raramente muda, então
  // busca uma vez só, sem polling.
  useEffect(() => {
    if (!statusToken) return
    let ativo = true
    fetch(`/api/pedido-app/pagamento-pix?token=${encodeURIComponent(statusToken)}`, { cache: 'no-store' })
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (ativo && data?.whatsappPizzaria) setWhatsappPizzaria(data.whatsappPizzaria)
      })
      .catch(() => {})
    return () => { ativo = false }
  }, [statusToken])
  ...
  return (
    <div style={{ background: 'var(--background)', minHeight: '100dvh', fontFamily: 'Archivo, sans-serif', color: 'var(--foreground)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: 'var(--surface)', borderTop: mostrarMapa ? '1px solid var(--surface-secondary)' : 'none', padding: '28px 20px calc(env(safe-area-inset-bottom) + 96px)', borderRadius: mostrarMapa ? '20px 20px 0 0' : 0, marginTop: mostrarMapa ? '-20px' : 0, position: 'relative', zIndex: 10, flex: mostrarMapa ? 'none' : 1 }}>
        ...
        {/* Botão WhatsApp — só aparece com número configurado (whatsappPizzaria);
            nunca um contato fictício/fixo. */}
        {whatsappPizzaria && (
          <a
            href={`https://wa.me/${whatsappPizzaria}`}
            target="_blank"
            rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', padding: '14px', borderRadius: '12px', background: 'var(--success)', color: 'var(--on-success)', fontSize: '15px', fontWeight: 700, textDecoration: 'none', boxSizing: 'border-box' }}
          >
