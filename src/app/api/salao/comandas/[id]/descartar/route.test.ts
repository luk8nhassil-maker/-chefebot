...
describe("POST /api/salao/comandas/[id]/descartar", () => {
  ...
  it("descarta um rascunho sem pedido enviado", async () => {
    ...
    expect(await buscarComanda(comanda.id)).toBeNull();
  });

  it("409 quando já enviou ao menos um pedido", async () => {
    const token = await criarTokenSalao();
    const comanda = await abrirAnonima();
    await marcarComandaEnviada(comanda.id, "ped_1", 1);
    const res = await POST(req(token), paramsFor(comanda.id));
    expect(res.status).toBe(409);
    expect(await buscarComanda(comanda.id)).not.toBeNull();
  });
